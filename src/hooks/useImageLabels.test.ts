...
describe("useImageLabels", () => {
  ...
  describe("toggleLabelByFilename", () => {
    it("should toggle label for specific image", async () => {
      ...
      await act(async () => {
        const batchResult =
          await result.current.toggleLabelByFilename("image2.jpg");
        expect(batchResult.success).toBe(true);
      });
    ...
    it("should fail for non-existent filename", async () => {
      ...
      await act(async () => {
        const batchResult =
          await result.current.toggleLabelByFilename("nonexistent.jpg");
        expect(batchResult.success).toBe(false);
  ...
  describe("batchToggleLabelByIndices", () => {
    it("should apply label to multiple indices", async () => {
      ...
      await act(async () => {
        const batchResult = await result.current.batchToggleLabelByIndices(
          [0, 2],
          "rejected",
        );
        expect(batchResult.success).toBe(true);
  ...
  describe("error handling with Promise.allSettled", () => {
    it("should handle partial failures gracefully", async () => {
      ...
      (setLabel as ReturnType<typeof vi.fn>).mockImplementation(
        (filename: string) => {
    ...
    it("should handle all failures", async () => {
      (setLabel as ReturnType<typeof vi.fn>).mockRejectedValue(
